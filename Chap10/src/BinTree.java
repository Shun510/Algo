...
class BinTree <K,V>{
	...
	private Comparator<? super K> comparator = null;	//コンバータ
	...
	public V search(K key){
		...
		while(true){
			if(p==null)							//これ以上進めなければ
				return null;					//…探索失敗
			int cond = comp(key,p.getKey());	//keyとノードｐのキーを比較
			if(cond ==0)						//等しければ
	...
	//　nodeを根とする部分木にノード<Ｋ，Ｖ>を挿入　//
	private void addNode(Node<K,V>node,K key,V data){
		int cond = comp(key,node.getKey());
		if(cond==0)
			return;		//keyは2分探索木上に既に存在
		else if (cond<0){
			if(node.left==null)
	...
	public boolean remove(K key){
		...
		if(p.left==null){						//pには左の子がない
			if(p==root)
				...
				parent.left=p.right;			//親の右ポインタが右の子を指す
			...
			p.key = left.key;					//leftのキー値をｐに移動
			p.data = left.data;					//leftのデータをｐに移動
			if(isLeftChild)
